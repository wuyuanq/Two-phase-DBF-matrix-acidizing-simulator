    ...
    subroutine Resi_tran_Cf(conc, resi)
        ...
        do j = indexd, indexu
            do i = indexl, indexr
                ...
                vwmodulus = dsqrt(vxw(i,j)**2.D0+vywAv**2.D0)
                ...
                dl = alphaOS*dm + 2.D0*lamdaX*vwmodulus*radiusInit*(1-poroEdgeXInit(i,j))/ &!
                    (poroEdgeXInit(i,j)*(1-poroEdgeX(i,j)))

                dt = alphaOS*dm + 2.D0*lamdaT*vwmodulus*radiusInit*(1-poroEdgeXInit(i,j))/ &!
                    (poroEdgeXInit(i,j)*(1-poroEdgeX(i,j)))

                Dx(i,j,1) = alphaOS*dm + vwmodulus/(poroEdgeX(i,j)*SwEdgeX(i,j))*((dl-dt)*Ex(i,j,1)+dt)
        ...
        do j = indexd, indexu
            do i = indexl, indexr
                ...
                dl = alphaOS*dm + 2.D0*lamdaX*vwmodulus*radiusInit*(1-poroEdgeYInit(i,j))/ &!
                    (poroEdgeYInit(i,j)*(1-poroEdgeY(i,j)))

                dt = alphaOS*dm + 2.D0*lamdaT*vwmodulus*radiusInit*(1-poroEdgeYInit(i,j))/ &!
                    (poroEdgeYInit(i,j)*(1-poroEdgeY(i,j)))

                Dy(i,j,1) = vwmodulus/(poroEdgeY(i,j)*SwEdgeY(i,j))*(dl-dt)*Ey(i,j,1)
                Dy(i,j,2) = alphaOS*dm + vwmodulus/(poroEdgeY(i,j)*SwEdgeY(i,j))*((dl-dt)*Ey(i,j,2)+dt)
        ...
        do j = 1, localnrows
            do i = 1, localncols
                ...
                reaction = Sw(i,j)*av(i,j)*conc(i,j)*kc(i,j)*ks/(kc(i,j)+ks)
